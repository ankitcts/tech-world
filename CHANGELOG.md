...
- **Official company logos crawled from Wikidata** (`pipeline/logos.py`): each
  ...
  **official logo** (`P154`, hosted on Wikimedia Commons), so the mark is tied
  ...
  (`TECHATLAS_LOGO_BATCH`, default 300) that stamps `logo_url` / `logo_source` /
  `logo_checked_at`; a ticker with no logo is recorded as a miss (not retried
  ahead of untouched companies, not fabricated). The endpoint **self-continues
  while logos remain** (not only while enrichment remains), so a single trigger
  drains the whole logo backlog across chained invocations. Run summary reports
  `logos_found` / `logos_checked` / `logos_remaining`. Needs `WIKIDATA_USER_AGENT` (falls back to
...
- Incorrect company logos: dropped the ticker-image logo services (parqet /
  ...
  only a **verified brand mark** (curated simple-icons); companies without one
  get an explicit **"no verified logo" placeholder** (a neutral broken-image
  glyph) rather than a guessed logo — consistent with the no-wrong-info rule.
- Live-data UI regressions on the landing page: (1) brand **logos** now render
  ...
  *enriched* with live SEC employee tiers + leadership (matched by ticker), while
  ...
  "disappear" when live data loads; (3) rows **loop only when their companies
  overflow the visible width** — a row that fits stays static — and sparse SIC
  domains are hidden until they have ≥3 companies to avoid dozens of one-company
