...
| `arch-gate.sh` | `UserPromptSubmit` | Architecture-first gate (soft): nudges to create `docs/architecture/` (C4 diagrams + docs via the architecture-diagrams agent) before a new project starts, or on first change to an existing project lacking them. Silent once docs exist. |
| `arch-precode.sh` | `PreToolUse` (Write\|Edit) | Architecture-BEFORE-build gate (hard): blocks writing application **source** code (exit 2) when the project has no `docs/architecture/architecture.md`, instructing the agent to prompt the user (AskUserQuestion) to create the architecture first or explicitly opt out. Exempts `docs/**`, `.claude/**`, tests, and non-source files; one-time per project. |
...
cp .claude/hooks/arch-precode.sh          ~/.claude/hooks/
...
```json
{
  "hooks": {
    ...
    "PreToolUse": [
      ...
      { "matcher": "Write|Edit", "hooks": [ { "type": "command", "command": "bash \"$HOME/.claude/hooks/arch-precode.sh\"" } ] }
    ],
    "PostToolUse": [
